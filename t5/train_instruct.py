...
def main():
    ...
    # 2. Configurations
    MODEL_NAME     = "VietAI/vit5-base"   # or another instruction-capable Seq2Seq
    DATASET_NAME   = "kienhoang123/Vietnamese_Poem_Analysis_VN"
    OUTPUT_DIR     = "./t5-vi-instruct-hf"
    BATCH_SIZE     = 4
    NUM_EPOCHS     = 3
    MAX_INPUT_LEN  = 512
    MAX_OUTPUT_LEN = 128
    ...
    def make_if(batch):
        ...
        output = " ||| ".join([
            str(batch["emotion_vi"] or ""),
            str(batch["metaphor_vi"] or ""),
            str(batch["setting_vi"] or ""),
            str(batch["motion_vi"] or ""),
            str(batch["prompt_vi"] or "")
        ])
        in_tok  = tokenizer(
